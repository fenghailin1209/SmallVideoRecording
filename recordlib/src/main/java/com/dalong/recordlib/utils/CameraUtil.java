...
public class CameraUtil {
    ...
    private CameraAscendSizeComparator_Height ascendSizeComparator_Height = new CameraAscendSizeComparator_Height();
...
//        activity.startActivityForResult(intent, AppConstant.REQUEST_CODE.CAMERA);
    ...
    public Bitmap setTakePicktrueOrientation(int cameraId, Bitmap bitmap) {
    ...
    public Size getPropPictureSize(List<Size> list, int minHeight) {
        Collections.sort(list, ascendSizeComparator);
     ...
     * @return
     */
    public Size getCommonPropSizeByHeightNew(List<Size> list, int minHeight) {
        Collections.sort(list, ascendSizeComparator_Height);
        int i = 0;
        for (Size s : list) {
            int height = s.height;
            float minWidth = height * 1.5f;//防止正方形
            Log.i(TAG, "--->>>getCommonPropSizeByHeight minWidth:" + minWidth);
    ...
    private void test(List<Size> list) {
        if (list != null) {
            for (Size size : list) {
                Log.i(TAG, "--->>>test size w:" + size.width + ",size h:" + size.height);
            }
        }
    }
     ...
     * @return
     */
    public boolean isVideoSizesContainPropSize(int preWidth, int PreHeight, List<Size> videoSizes) {
        if (videoSizes != null) {
    ...
    //升序
    public class CameraAscendSizeComparator_Height implements Comparator<Size> {
        public int compare(Size lhs, Size rhs) {
            if (lhs.height == rhs.height) {
                return 0;
            } else if (lhs.height > rhs.height) {
